var _Num_ = Contract.Base(function(x) {
  return (typeof x === "number"); 
});

var _Str_ = Contract.Base(function(x) {
  return (typeof x === "string"); 
});

var _Pos_ = Contract.Base(function(x) {
  return (typeof x === "number") && (x > 0); 
});

var _Even_ = Contract.Base(function(x) {
  return (typeof x === "number") && (x % 2 == 0); 
});


//1 @ _Pos_;
//Contract.assert(1, _Num_);

function plus(x, y) {
  return x+y;
}

//var plusNum = Contract.assert(plus, 
//    Contract.AFunction([_Pos_, _Pos_], _Pos_));
...
function addOne(x) {
  return Contract.assert(plus, Contract.AFunction([_Pos_, _Pos_], _Pos_)))(x, 1);
}


function addOne(x) {
  return Contract.assert(
      plus(Contract.assert(x, _Pos_), 1),
      _Pos_);
}
*/
/*
...
addOne = Contract.assert(addOne, Contract.AFunction([_Pos_], _Pos_));

//addOne(1)
*/

var target = {x:4711};
var proxy = new Observer(target, {get:function(t, n) { return 11;} });
...
/*
function test() {
  var addOneNum = Contract.assert(addOne, 
      Contract.AFunction([Contract.AFunction([_Pos_, _Pos_], _Pos_), _Pos_], _Pos_));
}
...
var plus$inter = Contract.Intersection(plus$number, plus$string);
var plusNum = Contract.assert(plus, plus$inter);
